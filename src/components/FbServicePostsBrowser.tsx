...
import { ImageIcon, Download, Check, Loader2, Globe, ChevronRight, Wrench } from "lucide-react";
...
const FbServicePostsBrowser = () => {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["fb-posts-services", activePage?.fb_page_id, afterCursor],
  ...
  const importMutation = useMutation({
    mutationFn: async (post: FbPost) => {
      ...
      const { data: { session } } = await supabase.auth.getSession();
      const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/import-fb-service-post`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${session?.access_token}`,
          apikey: import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY,
    ...
    onSuccess: () => {
      toast.success("Service post imported! Check Pending Review tab.");
      queryClient.invalidateQueries({ queryKey: ["fb-posts-services"] });
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          Import posts as services. AI reads the caption (image optional) to extract name, description and keywords.
        </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {posts.map((post) => {
          ...
          const hasText = !!(post.message && post.message.trim().length > 10);
          ...
          return (
            <Card
              ...
              } ${hasText && !post.already_imported ? "ring-2 ring-primary/30" : ""}`}
            >
              <div className="flex">
                <div className="w-28 h-28 bg-muted relative overflow-hidden shrink-0">
                  {post.image_url ? (
                  ...
                  {post.already_imported && (
                    <Badge variant="secondary" className="absolute top-1 left-1 text-[9px]">
                      <Check className="h-2.5 w-2.5 mr-1" /> Imported
                    </Badge>
                  )}
                </div>
                ...
                <CardContent className="p-3 flex-1 min-w-0 space-y-2">
                  <p className="text-xs text-muted-foreground line-clamp-3">
                    {post.message || "No caption — image-only post"}
                  </p>
                  ...
                  {post.already_imported ? (
                    ...
                    >
                      {isImporting ? (
                        ...
                        <><Download className="h-3 w-3" /> Import as Service</>
                      )}
                    </Button>
                  )}
                </CardContent>
...
export default FbServicePostsBrowser;
